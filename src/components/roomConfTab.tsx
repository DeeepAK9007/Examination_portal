...
const CustomButton = ({ rowData }: CustomButtonProps) => {
    const deleteUser = async (e: React.MouseEvent<HTMLElement>) => {
        ...
        const enc=btoa(targ);
        const resource= await fetch("http://localhost:8081/api/room?session_id="+seshID+"&resource="+enc+"&action=DELETE",
            {
              method: "POST",
              headers: {
...
function RoomConfTab()
{
    ...
    const [colDefs, setColDefs] = useState<ColDef<roomMatchedType, unknown>[]>([
        {field: "room_number",headerName: "Room Number",flex: 1,headerCheckboxSelection:true},
      ...
      return(
        <div>
            <div className="ag-theme-quartz mt-4 ms-5 shadow"style={{ height:400 , width:'90%'}}>
                <AgGridReact rowSelection="multiple" rowData={obtRooms} columnDefs={colDefs} />
            </div>
